...
describe("Mypage", () => {
  ...
  it("Mypage", () => {
    ...
    //비밀번호 입력
    cy.get(".css-1bq4gr1").type(Cypress.env("stgPW1"));
    ...
    cy.get("#password").clear().type(Cypress.env("stgPW1"));
    cy.get("#passwordConfirm").clear().type(Cypress.env("stgPW1"));
    cy.get(".css-2ptubj").contains("button", "확인").click();

    //Email 변경
    cy.get("#userEmail")
      .clear()
      .type(Cypress.env("stgEmail"));

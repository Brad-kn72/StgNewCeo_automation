...
describe("Basicmanagement", () => {
  ...
  it("Basicmanagement", () => {
    ...
    // cy.get('input[name="bizOwnerName"]').type(" ").type("{backspace}");
    // cy.get('input[name="bizOwnerName"]').type("1");
    // cy.get("img.css-r91awh").should("not.exist");
    // cy.intercept("PATCH", "https://staging-api.monkiceo.com/v1/stores/684/table-order").as("autoSave");
    // cy.wait("@autoSave");

    // // 수동 저장 → 최종 PATCH
    // cy.intercept("PATCH", "https://staging-api.monkiceo.com/v1/stores/684/table-order").as("finalSave");
    // cy.get(".css-1xtw0ps").contains("저장").click();

    // cy.wait("@finalSave").then(({ request }) => {
    //   expect(request.body.imgNos).to.be.undefined;
    // });

    // cy.get(
    //   ":nth-child(4) > :nth-child(2) > .css-1in8iz8 > .css-mehg5o > .css-yd8sa2 > .css-6tcmml > .css-1tfrujd > .css-ye8kfq"
    // ).click();
